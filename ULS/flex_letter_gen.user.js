...
(function() {
    ...
    function rbgCreateReportButton(buttonConfig) {
        ...
        if (buttonConfig.id === 'travelApprovalFloatingButton') {
            button.onclick = function() {
                ...
                initializeTravelApprovalModalLogic(currentPageDataForTravelModal.pjNum, currentPageDataForTravelModal.pjName);
    ...
    function initializeTravelApprovalModalLogic(pagePjNum = "N/A", pagePjName = "N/A") {
        ...
        function loadLocs(){const s=localStorage.getItem(storageKey);allLocations=s?JSON.parse(s):[...defaultLocations];if(!allLocations.includes("台北")){allLocations.unshift("台北");allLocations=[...new Set(allLocations)];}allLocations.sort((a,b)=>(a==="台北")?-1:(b==="台北")?1:a.localeCompare(b));}
        function saveLocs(){localStorage.setItem(storageKey,JSON.stringify(allLocations));}
        function addCustomLoc(){const v=newLocIn.value.trim();if(!v){alert('Enter loc.');return;}if(allLocations.some(l=>l.toLowerCase()===v.toLowerCase())){alert(`"${v}" exists.`);return;}allLocations.push(v);loadLocs();saveLocs();newLocIn.value='';alert(`"${v}" added.`);popLocDd(fromSelect,"台北");popLocDd(toSelect);}
        function resetLocs(){if(confirm("Reset locs & clear legs?")){allLocations=[...defaultLocations];saveLocs();selectedTripLegs=[];renderLegs();popLocDd(fromSelect,"台北");popLocDd(toSelect);alert("Locs reset.");}}
        ...
        function addLeg(){const f=fromSelect.value,t=toSelect.value;if(!f||!t){alert("Select From & To.");return;}if(f===t){alert("From & To same.");return;}selectedTripLegs.push({from:f,to:t});renderLegs();fromSelect.value="台北";toSelect.value="";}
        ...
        loadLocs();popLocDd(fromSelect,"台北");popLocDd(toSelect);renderCal(curMonth,curYear);renderLegs();
    }
