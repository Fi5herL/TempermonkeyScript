        ...
        For Each exportSheet In destWb.Worksheets
            If LCase(exportSheet.Name) Like "export*" Then
                ...
                If Not IsError(foundRow) Then
                    ...
                    exportSheet.Range("B2:F2").Value = Array("匯出時間", "會所中文名稱", "項目名稱", "狀態名稱", "原始檔案名稱")
                    exportSheet.Range("A2").Value = "會所名稱"
                    Debug.Print "    已填入第 2 行標題 (A2:F2)"

                    ' --- 填寫 第 3 列 的元數據值 (A3:E3) ---
                    exportSheet.Range("B3").Value = exportTimeVal ' 直接賦值保留格式
                    exportSheet.Range("C3").Value = huosuoName ' 使用中文會所名稱
                    exportSheet.Range("D3").Value = itemNameVal
                    exportSheet.Range("E3").Value = statusNameVal
                    exportSheet.Range("F3").Value = exportSheet.Name ' 分頁名即原始檔名
                    ' 嘗試格式化日期
                    On Error Resume Next
                    exportSheet.Range("B3").NumberFormat = "yyyy/m/d h:mm"
                    On Error GoTo 0
                    Debug.Print "    已填入第 3 行元數據值 (B3:F3)"
                    ...
                    ' 從 F3 開始向下填充
                    If lastRow >= 3 Then
                        exportSheet.Range("A3:A" & lastRow).Value = numericHallNumber ' 使用之前轉換好的數字
                        exportSheet.Range("B3:B" & lastRow).Value = exportTimeVal ' 使用之前轉換好的數字
                        exportSheet.Range("C3:C" & lastRow).Value = huosuoName ' 使用之前轉換好的數字
                        exportSheet.Range("D3:D" & lastRow).Value = itemNameVal ' 使用之前轉換好的數字
                        exportSheet.Range("E3:E" & lastRow).Value = statusNameVal ' 使用之前轉換好的數字
                        exportSheet.Range("F3:F" & lastRow).Value = exportSheet.Name ' 使用之前轉換好的數字
                        Debug.Print "    已將數字會所名稱填入 A3:A" & lastRow
                    End If
                ...
                Else
                    ...
                    ' --- 仍然插入 A:F 列並填寫標題和標記 ---
                    exportSheet.Columns("A:F").Insert Shift:=xlToRight
                    exportSheet.Range("B2:F2").Value = Array("匯出時間", "會所名稱", "項目名稱", "狀態名稱", "原始檔案名稱")
                    exportSheet.Range("A2").Value = "會所名稱"
                    exportSheet.Range("B3:F3").Value = Array("N/A", "未找到", "N/A", "N/A", exportSheet.Name) ' 標示未找到
                    exportSheet.Range("A3").Value = "未找到" ' 在 F3 標示
