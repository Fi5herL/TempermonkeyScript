// ==UserScript==
// @name         Flex 📁 Folder Copy + Notes + Calendar
// @namespace    fisher-flex-folder-notes-calendar
// @version      2.1.0
// @description  在 Flex Dashboard 加上資料夾複製、快速筆記與行事曆檢視功能。
...
(function() {
    ...
    GM_addStyle(`
        ...
        .ffn-view-tab-btn:hover{ background:#eef2ff; color:#3730a3; }
        .ffn-view-tab-btn.active{
            background:#4f46e5;
            color:#fff;
        }
        .ffn-close-btn{
        ...
        .ffn-calendar-view[hidden]{ display:none !important; }
        .ffn-calendar-toolbar{
            display:flex;
            align-items:center;
            justify-content:space-between;
            margin-bottom:12px;
        ...
        .ffn-cal-day{
            ...
            animation:ffn-fade-in .2s ease-out;
        }
        .ffn-cal-day:hover{
            border-color:#4f46e5;
            box-shadow:0 0 0 1px rgba(79,70,229,.15);
        }
        .ffn-cal-day.ffn-cal-other-month{
            background:#f9fafb;
            color:#9ca3af;
        }
        .ffn-cal-day.ffn-cal-today{
            border-color:#4f46e5;
        ...
        .ffn-cal-chip-priority-high{ background:#fee2e2; color:#b91c1c; border-color:#fecaca; }
        .ffn-cal-chip-priority-medium{ background:#fef3c7; color:#92400e; border-color:#fde68a; }
        .ffn-cal-chip-priority-low{ background:#dcfce7; color:#166534; border-color:#bbf7d0; }
        .ffn-cal-chip-tag-urgent{ background:#fee2e2; color:#b91c1c; border-color:#fecaca; }
        .ffn-cal-chip-tag-followup{ background:#e0e7ff; color:#3730a3; border-color:#c7d2fe; }
        .ffn-cal-chip-more{
            font-size:11px;
            color:#6b7280;
            padding-left:4px;
        }
        .ffn-day-popup-backdrop{
            position:fixed;
            inset:0;
            z-index:99998;
            background:rgba(0,0,0,.2);
        ...
        .ffn-day-popup-edit-btn:hover{ background:#4338ca; }
    ...
    function getCalendarNotesMap() {
        const map = new Map();
        loadStore().notes.forEach(note => {
    ...
    function getCalendarChipClass(note) {
        const priority = String(note.priority || '').toLowerCase();
        if (priority === 'high') return 'ffn-cal-chip-priority-high';
        if (priority === 'medium') return 'ffn-cal-chip-priority-medium';
        if (priority === 'low') return 'ffn-cal-chip-priority-low';
        const tags = parseTagsInput(note.tags).map(tag => tag.toLowerCase());
        if (tags.some(tag => ['urgent', 'critical', 'high'].includes(tag))) return 'ffn-cal-chip-tag-urgent';
        if (tags.some(tag => ['followup', 'follow-up', 'todo'].includes(tag))) return 'ffn-cal-chip-tag-followup';
        return '';
    }

    function getNoteChipText(note) {
        const firstLine = String(note.text || '').split('\n').find(line => line.trim()) || '(空白筆記)';
    ...
    function openDayPopup(dateKey, notes) {
        closeDayPopup();
        const backdrop = document.createElement('div');
        backdrop.className = 'ffn-day-popup-backdrop';
        backdrop.addEventListener('click', (e) => {
            if (e.target === backdrop) closeDayPopup();
        });

        const popup = document.createElement('div');
        popup.className = 'ffn-day-popup';

        const popupHeader = document.createElement('div');
        popupHeader.className = 'ffn-day-popup-header';
        const title = document.createElement('div');
        title.className = 'ffn-day-popup-title';
        title.textContent = `📅 ${dateKey}（${notes.length} 筆）`;
        const closeBtn = document.createElement('button');
        closeBtn.type = 'button';
        closeBtn.className = 'ffn-close-btn';
        closeBtn.title = '關閉';
        ...
        list.className = 'ffn-day-popup-list';

        if (!notes.length) {
            const empty = document.createElement('div');
            empty.className = 'ffn-empty-state';
            empty.innerHTML = '<div class="ffn-empty-state-icon">📭</div><div>當天沒有筆記</div>';
            list.appendChild(empty);
        } else {
            notes.forEach(note => {
                const item = document.createElement('div');
                item.className = 'ffn-day-popup-note';
                const meta = document.createElement('span');
                meta.textContent = `📁 ${note.projectNumber || '-'} · ${note.fileNo || '-'} · ${formatDateTime(note.createdAt)}`;
                const editBtn = document.createElement('button');
                editBtn.type = 'button';
                editBtn.className = 'ffn-day-popup-edit-btn';
                editBtn.textContent = '開啟編輯';
                editBtn.addEventListener('click', () => openNoteEditor({ note }));
                const metaWrap = document.createElement('div');
                metaWrap.className = 'ffn-day-popup-meta';
                metaWrap.append(meta, editBtn);
                const content = document.createElement('div');
                content.className = 'ffn-memo-content';
                content.innerHTML = renderMarkdown(note.text || '') || PREVIEW_PLACEHOLDER_HTML;
                item.append(metaWrap, content);
    ...
    function renderCalendarView() {
        ...
        const titleEl = notesPanelEl.querySelector('#ffn-calendar-title');
        ...
        weekdays.forEach(weekday => {
            ...
            dayHeader.className = 'ffn-cal-weekday';
        ...
        const monthDays = new Date(calState.year, calState.month + 1, 0).getDate();
        const totalCells = Math.ceil((startOffset + monthDays) / 7) * 7;
        const notesMap = getCalendarNotesMap();
        const todayKey = getDateKey(new Date());

        for (let i = 0; i < totalCells; i += 1) {
            const dayDate = new Date(calState.year, calState.month, 1 - startOffset + i);
            const inCurrentMonth = dayDate.getMonth() === calState.month;
            const dayKey = getDateKey(dayDate);
            const dayNotes = notesMap.get(dayKey) || [];

            const dayCell = document.createElement('button');
            dayCell.type = 'button';
            dayCell.className = 'ffn-cal-day';
            if (!inCurrentMonth) dayCell.classList.add('ffn-cal-other-month');
            if (dayKey === todayKey) dayCell.classList.add('ffn-cal-today');
            dayCell.title = `${dayKey}（${dayNotes.length} 筆）`;

            const dayNumber = document.createElement('div');
            dayNumber.className = 'ffn-cal-day-number';
            dayNumber.textContent = String(dayDate.getDate());

            const chipList = document.createElement('div');
            chipList.className = 'ffn-cal-chip-list';
            dayNotes.slice(0, 3).forEach(note => {
                const chip = document.createElement('div');
                const colorClass = getCalendarChipClass(note);
                chip.className = colorClass ? `ffn-cal-chip ${colorClass}` : 'ffn-cal-chip';
                chip.textContent = getNoteChipText(note);
                chipList.appendChild(chip);
            });
            if (dayNotes.length > 3) {
                const more = document.createElement('div');
                more.className = 'ffn-cal-chip-more';
                more.textContent = `＋${dayNotes.length - 3} 筆`;
                chipList.appendChild(more);
            }

            dayCell.append(dayNumber, chipList);
            dayCell.addEventListener('click', () => openDayPopup(dayKey, dayNotes));
            grid.appendChild(dayCell);
        }
    }

    function updateMainHeaderTitle(noteCount) {
        if (!notesPanelEl) return;
        const headerTitle = notesPanelEl.querySelector('.ffn-main-header-title');
        if (!headerTitle) return;
        headerTitle.textContent = currentPanelTab === 'calendar'
            ? '📅 行事曆'
            : `所有筆記（${noteCount}）`;
    }
    ...
    function setPanelTab(tab) {
        if (!notesPanelEl) return;
        currentPanelTab = tab === 'calendar' ? 'calendar' : 'notes';
        const notesList = notesPanelEl.querySelector('.ffn-notes-list');
        const calendarView = notesPanelEl.querySelector('.ffn-calendar-view');
        const notesTabBtn = notesPanelEl.querySelector('[data-ffn-tab="notes"]');
        const calendarTabBtn = notesPanelEl.querySelector('[data-ffn-tab="calendar"]');
        if (notesList) notesList.hidden = currentPanelTab !== 'notes';
        if (calendarView) calendarView.hidden = currentPanelTab !== 'calendar';
        if (notesTabBtn) notesTabBtn.classList.toggle('active', currentPanelTab === 'notes');
        if (calendarTabBtn) calendarTabBtn.classList.toggle('active', currentPanelTab === 'calendar');
        if (currentPanelTab === 'calendar') renderCalendarView();
        updateMainHeaderTitle(getFilteredSortedNotes().length);
    }
    ...
    function renderNotesList() {
        ...
        // Update header count
        updateMainHeaderTitle(notes.length);
    ...
    function createNotesPanel() {
        ...
        notesPanelEl.innerHTML = `
            ...
            <div class="ffn-main">
                <div class="ffn-main-header">
                    <div class="ffn-main-header-left">
                        <span class="ffn-main-header-title">所有筆記</span>
                        <div class="ffn-view-tabs">
                            <button type="button" class="ffn-view-tab-btn active" data-ffn-tab="notes">📝 筆記</button>
                            <button type="button" class="ffn-view-tab-btn" data-ffn-tab="calendar">📅 行事曆</button>
                        </div>
                    </div>
                    <button type="button" id="ffn-close-panel" class="ffn-close-btn" title="關閉">✕</button>
                </div>
                <div class="ffn-notes-list"></div>
                <div class="ffn-calendar-view" hidden>
                    <div class="ffn-calendar-toolbar">
                        <div class="ffn-calendar-toolbar-left">
                            <button type="button" class="ffn-calendar-nav-btn" id="ffn-cal-prev">◀ 上月</button>
                            <button type="button" class="ffn-calendar-nav-btn" id="ffn-cal-next">下月 ▶</button>
                            <button type="button" class="ffn-calendar-nav-btn" id="ffn-cal-today">今天</button>
                        </div>
                        <div class="ffn-calendar-title" id="ffn-calendar-title"></div>
                    </div>
                    <div class="ffn-calendar-grid" id="ffn-calendar-grid"></div>
                </div>
            </div>
        `;
        ...
        notesOverlayEl.addEventListener('click', (e) => {
            if (e.target === notesOverlayEl) {
                closeDayPopup();
                notesOverlayEl.remove();
            }
        });

        notesPanelEl.querySelector('#ffn-close-panel').addEventListener('click', () => {
            closeDayPopup();
            notesOverlayEl.remove();
        });
        ...
        notesPanelEl.querySelectorAll('input,select').forEach(el => el.addEventListener('input', () => {
            renderNotesList();
            if (currentPanelTab === 'calendar') renderCalendarView();
        }));

        notesPanelEl.querySelector('[data-ffn-tab="notes"]').addEventListener('click', () => setPanelTab('notes'));
        notesPanelEl.querySelector('[data-ffn-tab="calendar"]').addEventListener('click', () => setPanelTab('calendar'));
        notesPanelEl.querySelector('#ffn-cal-prev').addEventListener('click', () => {
            calState.month -= 1;
            if (calState.month < 0) {
                calState.month = 11;
                calState.year -= 1;
        ...
        notesPanelEl.querySelector('#ffn-cal-next').addEventListener('click', () => {
            calState.month += 1;
        ...
        notesPanelEl.querySelector('#ffn-cal-today').addEventListener('click', () => {
            const now = new Date();
